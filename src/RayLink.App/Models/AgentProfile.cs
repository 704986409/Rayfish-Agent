...
public sealed class AgentProfile : INotifyPropertyChanged
{
    ...
    private void Set<T>(ref T field,T value,[CallerMemberName]string? n=null){if(EqualityComparer<T>.Default.Equals(field,value))return;field=value;PropertyChanged?.Invoke(this,new PropertyChangedEventArgs(n)); if(n==nameof(IsOnline))PropertyChanged?.Invoke(this,new PropertyChangedEventArgs(nameof(Status)));}
}
